...
describe("Territory Controller", () => {
	...
	describe(".getAll()", () => {
		...
		it("should group territorys by type field", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData.data).to.have.property("territorys");
			expect(sentData.data).to.have.property("groupedBy", "type");
			expect(sentData.data.territorys).to.have.property("email");
			expect(sentData.data.territorys).to.have.property("sms");
			expect(sentData.data.territorys).to.have.property("unassigned");
		});
		...
		it("should group territorys by name field", async function () {
			...
			expect(sentData.data.territorys).to.have.property("User Registration Territory");
			expect(sentData.data.territorys).to.have.property("SMS Notification Territory");
		});
		...
		it("should handle territorys with null values in grouping field", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData.data.territorys).to.have.property("unassigned");
			expect(sentData.data.territorys.unassigned).to.be.an("array");
			expect(sentData.data.territorys.unassigned.length).to.be.greaterThan(0);
		});
		...
		it("should return normal response when groupBy is not provided", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData.data.territorys).to.be.an("array");
	...
	describe(".getById()", () => {
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.territory.findFirst = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".update()", () => {
		it("should update territory details", async function () {
			...
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data.territory).to.have.property("id");
		});
