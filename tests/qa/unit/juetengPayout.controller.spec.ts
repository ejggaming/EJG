...
describe("JuetengPayout Controller", () => {
	...
	describe(".getAll()", () => {
		...
		it("should group juetengPayouts by type field", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData.data).to.have.property("juetengPayouts");
			expect(sentData.data).to.have.property("groupedBy", "type");
			expect(sentData.data.juetengPayouts).to.have.property("email");
			expect(sentData.data.juetengPayouts).to.have.property("sms");
			expect(sentData.data.juetengPayouts).to.have.property("unassigned");
		});
		...
		it("should group juetengPayouts by name field", async function () {
			...
			expect(sentData.data.juetengPayouts).to.have.property("User Registration JuetengPayout");
			expect(sentData.data.juetengPayouts).to.have.property("SMS Notification JuetengPayout");
		});
		...
		it("should handle juetengPayouts with null values in grouping field", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData.data.juetengPayouts).to.have.property("unassigned");
			expect(sentData.data.juetengPayouts.unassigned).to.be.an("array");
			expect(sentData.data.juetengPayouts.unassigned.length).to.be.greaterThan(0);
		});
		...
		it("should return normal response when groupBy is not provided", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData.data.juetengPayouts).to.be.an("array");
	...
	describe(".getById()", () => {
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.juetengPayout.findFirst = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".create()", () => {
		it("should create a new juetengPayout", async function () {
			...
			betId: "507f1f77bcf86cd799439100",
			drawId: "507f1f77bcf86cd799439101",
			bettorId: "507f1f77bcf86cd799439102",
			amount: 100,
		};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update juetengPayout details", async function () {
			...
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data.juetengPayout).to.have.property("id");
		});
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long juetengPayout name", async function () {
			...
			const createData = {
				...
				betId: "507f1f77bcf86cd799439100",
				drawId: "507f1f77bcf86cd799439101",
				bettorId: "507f1f77bcf86cd799439011",
				amount: 100,
			};
			req.body = createData;
			await juetengPayoutController.create(req as Request, res, next);
