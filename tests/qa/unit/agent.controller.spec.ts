import { controller } from "../../../app/agent/agent.controller";
...
describe("Agent Controller", () => {
	...
	const mockAgent = {
		...
		description: "Agent for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockAgents = [
		...
		},
		{
			...
			description: "Agent for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Agent for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			agent: {
				findMany: async (_params: Prisma.AgentFindManyArgs) => {
					// Return multiple agents for grouping tests
					if (req.query?.groupBy) {
						return mockAgents;
					}
					return [mockAgent];
				},
				count: async (_params: Prisma.AgentCountArgs) => {
				...
				findFirst: async (params: Prisma.AgentFindFirstArgs) =>
					params.where?.id === mockAgent.id ? mockAgent : null,
				findUnique: async (params: Prisma.AgentFindUniqueArgs) =>
					params.where?.id === mockAgent.id ? mockAgent : null,
				create: async (params: Prisma.AgentCreateArgs) => ({
					...mockAgent,
					...params.data,
				}),
				update: async (params: Prisma.AgentUpdateArgs) => ({
					...mockAgent,
					...params.data,
				}),
				delete: async (params: Prisma.AgentDeleteArgs) => ({
					...mockAgent,
					id: params.where.id,
				}),
			},
		...
		agentController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated agents", async function () {
			...
			await agentController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group agents by type field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type", document: "true" };
			await agentController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData.data).to.have.property("agents");
			expect(sentData.data).to.have.property("groupedBy", "type");
			expect(sentData.data.agents).to.have.property("email");
			expect(sentData.data.agents).to.have.property("sms");
			expect(sentData.data.agents).to.have.property("unassigned");
		});

		it("should group agents by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.agents).to.have.property("User Registration Agent");
			expect(sentData.data.agents).to.have.property("SMS Notification Agent");
		});

		it("should handle agents with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type", document: "true" };
			await agentController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData.data.agents).to.have.property("unassigned");
			expect(sentData.data.agents.unassigned).to.be.an("array");
			expect(sentData.data.agents.unassigned.length).to.be.greaterThan(0);
		});
		...
		it("should return normal response when groupBy is not provided", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData.data.agents).to.be.an("array");
	...
	describe(".getById()", () => {
		it("should return a agent", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockAgent.id };
			await agentController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockAgent.id });
		});
		...
		it("should handle non-existent agent", async function () {
			this.timeout(TEST_TIMEOUT);
	...
	describe(".create()", () => {
		it("should create a new agent", async function () {
			...
			const createData = {
				...
				description: "Agent for contact forms with validation",
			userId: "507f1f77bcf86cd799439100",
			role: "COBRADOR",
		};
			req.body = createData;
			await agentController.create(req as Request, res, next);
		...
		it("should create a new agent with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Agent",
				description: "Agent for email notifications",
				type: "email",
			userId: "507f1f77bcf86cd799439100",
			role: "COBRADOR",
		};
		...
		it("should create a new agent without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Agent",
				description: "Agent without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Agent",
				description: "Agent from form data",
				type: "form",
			userId: "507f1f77bcf86cd799439100",
			role: "COBRADOR",
		};
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Agent",
				description: "Agent from URL encoded data",
			userId: "507f1f77bcf86cd799439100",
			role: "COBRADOR",
		};
			req.body = createData;
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Agent",
				description: "Agent that will cause Prisma error",
			userId: "507f1f77bcf86cd799439100",
			role: "COBRADOR",
		};
			req.body = createData;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Agent",
				description: "Agent that will cause internal error",
			userId: "507f1f77bcf86cd799439100",
			role: "COBRADOR",
		};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update agent details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Agent",
				description: "Updated agent with additional validation and styling options",
			};
			req.params = { id: mockAgent.id };
			req.body = updateData;
			await agentController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data.agent).to.have.property("id");
		});

		it("should update agent type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple agent fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Agent",
		...
		it("should handle non-existent agent update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Agent",
			};
	...
	describe(".remove()", () => {
		it("should delete a agent", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockAgent.id };
			await agentController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent agent deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await agentController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long agent name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle special characters in agent data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
		...
		it("should handle concurrent requests", async function () {
			...
			const promises = Array(5)
				.fill(null)
				.map(() => agentController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Agent 1", type: "email", category: "marketing" },
		{ id: 2, name: "Agent 2", type: "sms", category: "notification" },
		{ id: 3, name: "Agent 3", type: "email", category: "marketing" },
		{ id: 4, name: "Agent 4", type: null, category: "general" },
		{ id: 5, name: "Agent 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Agent 1", type: "email" },
				{ id: 2, name: "Agent 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Agent 1");
			expect(result).to.have.property("Agent 2");
			expect(result).to.have.property("Agent 3");
			expect(result).to.have.property("Agent 4");
			expect(result).to.have.property("Agent 5");
			expect(result["Agent 1"]).to.have.length(1);
		});
	});
});
