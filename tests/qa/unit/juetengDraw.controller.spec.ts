...
describe("JuetengDraw Controller", () => {
	...
	describe(".getAll()", () => {
		...
		it("should group juetengDraws by type field", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData.data).to.have.property("juetengDraws");
			expect(sentData.data).to.have.property("groupedBy", "type");
			expect(sentData.data.juetengDraws).to.have.property("email");
			expect(sentData.data.juetengDraws).to.have.property("sms");
			expect(sentData.data.juetengDraws).to.have.property("unassigned");
		});
		...
		it("should group juetengDraws by name field", async function () {
			...
			expect(sentData.data.juetengDraws).to.have.property("User Registration JuetengDraw");
			expect(sentData.data.juetengDraws).to.have.property("SMS Notification JuetengDraw");
		});
		...
		it("should handle juetengDraws with null values in grouping field", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData.data.juetengDraws).to.have.property("unassigned");
			expect(sentData.data.juetengDraws.unassigned).to.be.an("array");
			expect(sentData.data.juetengDraws.unassigned.length).to.be.greaterThan(0);
		});
		...
		it("should return normal response when groupBy is not provided", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData.data.juetengDraws).to.be.an("array");
	...
	describe(".create()", () => {
		it("should create a new juetengDraw", async function () {
			...
			scheduleId: "507f1f77bcf86cd799439100",
			drawDate: "2026-01-01",
			drawType: "MORNING",
	...
	describe(".update()", () => {
		it("should update juetengDraw details", async function () {
			...
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data.juetengDraw).to.have.property("id");
		});
		...
		it("should handle validation errors", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = { drawType: "INVALID_TYPE" };
		req.params = { id: mockJuetengDraw.id };
			req.body = updateData;
			await juetengDrawController.update(req as Request, res, next);
			expect(statusCode).to.equal(400);
