...
describe("Wallet Controller", () => {
	...
	const mockWallet = {
		...
		name: "User Registration Wallet",
		description: "Wallet for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockWallets = [
		...
		},
		{
			...
			name: "SMS Notification Wallet",
			description: "Wallet for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Email Marketing Wallet",
			description: "Wallet for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Generic Wallet",
			description: "Wallet without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			wallet: {
				findMany: async (_params: Prisma.WalletFindManyArgs) => {
					// Return multiple wallets for grouping tests
					if (req.query?.groupBy) {
						return mockWallets;
					}
					return [mockWallet];
				},
				count: async (_params: Prisma.WalletCountArgs) => {
					// Return count based on whether grouping is requested
					if (req.query?.groupBy) {
						return mockWallets.length;
					}
					return 1;
				},
				findFirst: async (params: Prisma.WalletFindFirstArgs) =>
					params.where?.id === mockWallet.id ? mockWallet : null,
				findUnique: async (params: Prisma.WalletFindUniqueArgs) =>
					params.where?.id === mockWallet.id ? mockWallet : null,
				create: async (params: Prisma.WalletCreateArgs) => ({
					...mockWallet,
					...params.data,
				}),
				update: async (params: Prisma.WalletUpdateArgs) => ({
					...mockWallet,
					...params.data,
				}),
				delete: async (params: Prisma.WalletDeleteArgs) => ({
					...mockWallet,
					id: params.where.id,
				}),
			},
	...
	describe(".getAll()", () => {
		it("should return paginated wallets", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10", document: "true", count: "true" };
			await walletController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group wallets by type field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type", document: "true" };
			await walletController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData.data).to.have.property("wallets");
			expect(sentData.data).to.have.property("groupedBy", "type");
			expect(sentData.data.wallets).to.have.property("email");
			expect(sentData.data.wallets).to.have.property("sms");
			expect(sentData.data.wallets).to.have.property("unassigned");
		});

		it("should group wallets by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.wallets).to.have.property("User Registration Wallet");
			expect(sentData.data.wallets).to.have.property("SMS Notification Wallet");
		});

		it("should handle wallets with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type", document: "true" };
			await walletController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData.data.wallets).to.have.property("unassigned");
			expect(sentData.data.wallets.unassigned).to.be.an("array");
			expect(sentData.data.wallets.unassigned.length).to.be.greaterThan(0);
		});
		...
		it("should return normal response when groupBy is not provided", async function () {
			...
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData.data.wallets).to.be.an("array");
	...
	describe(".getById()", () => {
		it("should return a wallet", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockWallet.id };
			await walletController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockWallet.id });
		});
	...
	describe(".create()", () => {
		it("should create a new wallet", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Wallet",
				description: "Wallet for contact forms with validation",
			userId: "507f1f77bcf86cd799439100",
		};
			req.body = createData;
			await walletController.create(req as Request, res, next);
		...
		it("should create a new wallet with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Wallet",
				description: "Wallet for email notifications",
				type: "email",
			userId: "507f1f77bcf86cd799439100",
		};
			req.body = createData;
		...
		it("should create a new wallet without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Wallet",
				description: "Wallet without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Wallet",
				description: "Wallet from form data",
				type: "form",
			userId: "507f1f77bcf86cd799439100",
		};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Wallet",
				description: "Wallet from URL encoded data",
			userId: "507f1f77bcf86cd799439100",
		};
			req.body = createData;
			(req as any).get = (header: string) => {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Wallet with empty name",
			};
			req.body = createData;
			await walletController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Wallet",
				description: "Wallet that will cause Prisma error",
			userId: "507f1f77bcf86cd799439100",
		};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.wallet.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Wallet",
				description: "Wallet that will cause internal error",
			userId: "507f1f77bcf86cd799439100",
		};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update wallet details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Wallet",
				description: "Updated wallet with additional validation and styling options",
			};
			req.params = { id: mockWallet.id };
			req.body = updateData;
			await walletController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data.wallet).to.have.property("id");
		});

		it("should update wallet type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple wallet fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Wallet",
		...
		it("should handle non-existent wallet update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Wallet",
			};
	...
	describe(".remove()", () => {
		it("should delete a wallet", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockWallet.id };
			await walletController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent wallet deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await walletController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long wallet name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Wallet with very long name",
		...
		it("should handle special characters in wallet data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Wallet with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Wallet created concurrently",
			userId: "507f1f77bcf86cd799439100",
		};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => walletController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Wallet 1", type: "email", category: "marketing" },
		{ id: 2, name: "Wallet 2", type: "sms", category: "notification" },
		{ id: 3, name: "Wallet 3", type: "email", category: "marketing" },
		{ id: 4, name: "Wallet 4", type: null, category: "general" },
		{ id: 5, name: "Wallet 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Wallet 1", type: "email" },
				{ id: 2, name: "Wallet 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Wallet 1");
			expect(result).to.have.property("Wallet 2");
			expect(result).to.have.property("Wallet 3");
			expect(result).to.have.property("Wallet 4");
			expect(result).to.have.property("Wallet 5");
			expect(result["Wallet 1"]).to.have.length(1);
		});
	});
});
