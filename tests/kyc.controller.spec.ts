...
describe("Kyc Controller", () => {
	...
	beforeEach(() => {
		prisma = {
			kyc: {
				findMany: async (_params: Prisma.KycFindManyArgs) => {
				...
				count: async (_params: Prisma.KycCountArgs) => {
				...
				findFirst: async (params: Prisma.KycFindFirstArgs) =>
					params.where?.id === mockKyc.id ? mockKyc : null,
				findUnique: async (params: Prisma.KycFindUniqueArgs) =>
					params.where?.id === mockKyc.id ? mockKyc : null,
				create: async (params: Prisma.KycCreateArgs) => ({
					...mockKyc,
					...params.data,
				}),
				update: async (params: Prisma.KycUpdateArgs) => ({
					...mockKyc,
					...params.data,
				}),
				delete: async (params: Prisma.KycDeleteArgs) => ({
					...mockKyc,
					id: params.where.id,
				}),
			},
	...
	describe(".getAll()", () => {
		...
		it("should group kycs by name field", async function () {
			...
			expect(sentData.data.grouped).to.have.property("User Registration Kyc");
			expect(sentData.data.grouped).to.have.property("SMS Notification Kyc");
		});
	...
	describe(".create()", () => {
		it("should create a new kyc", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Kyc",
				description: "Kyc for contact forms with validation",
			};
			req.body = createData;
		...
		it("should create a new kyc with type field", async function () {
			...
			const createData = {
				name: "Email Kyc",
				description: "Kyc for email notifications",
				type: "email",
			};
			req.body = createData;
			await kycController.create(req as Request, res, next);
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Kyc",
				description: "Kyc from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Kyc",
				description: "Kyc from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Kyc with empty name",
			};
			req.body = createData;
			await kycController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Kyc",
				description: "Kyc that will cause Prisma error",
			};
			req.body = createData;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Kyc",
				description: "Kyc that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update kyc details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Kyc",
				description: "Updated kyc with additional validation and styling options",
			};
			req.params = { id: mockKyc.id };
			req.body = updateData;
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long kyc name", async function () {
			...
			const createData = {
				...
				description: "Kyc with very long name",
			};
			req.body = createData;
			await kycController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				name: "Concurrent Kyc",
				description: "Kyc created concurrently",
			};
			req.body = createData;
