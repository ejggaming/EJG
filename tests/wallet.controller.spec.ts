...
describe("Wallet Controller", () => {
	...
	describe(".getAll()", () => {
		...
		it("should group wallets by name field", async function () {
			...
			expect(sentData.data.grouped).to.have.property("User Registration Wallet");
			expect(sentData.data.grouped).to.have.property("SMS Notification Wallet");
		});
	...
	describe(".getById()", () => {
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.wallet.findUnique = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
