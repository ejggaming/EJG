...
describe("Commission Controller", () => {
	...
	beforeEach(() => {
		prisma = {
			commission: {
				findMany: async (_params: Prisma.CommissionFindManyArgs) => {
				...
				count: async (_params: Prisma.CommissionCountArgs) => {
				...
				findFirst: async (params: Prisma.CommissionFindFirstArgs) =>
					params.where?.id === mockCommission.id ? mockCommission : null,
				findUnique: async (params: Prisma.CommissionFindUniqueArgs) =>
					params.where?.id === mockCommission.id ? mockCommission : null,
				create: async (params: Prisma.CommissionCreateArgs) => ({
					...mockCommission,
					...params.data,
				}),
				update: async (params: Prisma.CommissionUpdateArgs) => ({
					...mockCommission,
					...params.data,
				}),
				delete: async (params: Prisma.CommissionDeleteArgs) => ({
					...mockCommission,
					id: params.where.id,
				}),
			},
	...
	describe(".getAll()", () => {
		...
		it("should group commissions by name field", async function () {
			...
			expect(sentData.data.grouped).to.have.property("User Registration Commission");
			expect(sentData.data.grouped).to.have.property("SMS Notification Commission");
		});
	...
	describe(".getById()", () => {
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.commission.findUnique = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".create()", () => {
		it("should create a new commission", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Commission",
				description: "Commission for contact forms with validation",
			};
			req.body = createData;
		...
		it("should create a new commission with type field", async function () {
			...
			const createData = {
				...
				description: "Commission for email notifications",
				type: "email",
			};
			req.body = createData;
			await commissionController.create(req as Request, res, next);
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				...
				description: "Commission from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Commission",
				description: "Commission from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Commission with empty name",
			};
			req.body = createData;
			await commissionController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				...
				description: "Commission that will cause Prisma error",
			};
			req.body = createData;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Commission",
				description: "Commission that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update commission details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Commission",
				description: "Updated commission with additional validation and styling options",
			};
			req.params = { id: mockCommission.id };
			req.body = updateData;
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long commission name", async function () {
			...
			const createData = {
				...
				description: "Commission with very long name",
			};
			req.body = createData;
			await commissionController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in commission data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Commission with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Concurrent Commission",
				description: "Commission created concurrently",
			};
			req.body = createData;
