...
describe("JuetengBet Controller", () => {
	...
	describe(".getAll()", () => {
		...
		it("should group juetengBets by name field", async function () {
			...
			expect(sentData.data.grouped).to.have.property("User Registration JuetengBet");
			expect(sentData.data.grouped).to.have.property("SMS Notification JuetengBet");
		});
	...
	describe(".getById()", () => {
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.juetengBet.findUnique = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
